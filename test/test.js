...
var dba = require('bindings')('db2ia');
...
sqlAs.execSync(sql, function(error, x) {
  console.log('Execute A Done. Row Count: %d', x.length);
  sqlAs.close();
});
console.log('Execute B.');
sqlBs.execSync(sql, function(error, x) {
...
console.log('Execute A.');
sqlA.exec(sql, function(error, x) {
  console.log('Execute A Done. Row Count: %d', x.length);
  sqlA.close();
  console.log('Execute B.');
  sqlB.exec(sql, function(error, x) {
    console.log('Execute B Done. Row Count: %d', x.length);
    sqlB.close();
    dbconn.disconn();
    dbconn.close();
